...
function getAllHistory(req, res, con) {
  ...
  var sql =
    "select * from complaint_info c,handyman_info h,student_info s where c.handyman_id=h.handyman_id and c.student_id=s.student_id ";
  if (handyman_id != "") {
  ...
  if (status != "Any") {
    if (status == "Lodged") status = "(0)";
    if (status == "Unsolved") status = "(0,1)";
    if (status == "Progress") status = "(1)";
    if (status == "Solved") status = "(2,3)";
    if (status == "Closed") status = "(3)";
    sql += "and status in " + status + " ";
  }
  if (rating != "Any") {
    if (rating == "Poor") rating = "(1,2)";
    if (rating == "Average") rating = "(3)";
    if (rating == "Good") rating = "(4,5)";
...
function assignWorkHelper(Data, slot, result, status, res, con) {
  ...
  if (min >= 5) return -1;
    ...
    if (status == 0)
      var sql =
        "update complaint_info set handyman_id=" +
        id +
        ", status=1 ,assignDate=NOW() where complaint_id=" +
        result.complaint_id;
    else
      var sql =
        "update complaint_info set handyman_id=" +
        id +
        ", status=1 where complaint_id=" +
        result.complaint_id;
    con.query(sql, function (err, result) {
      if (err) {
        console.log(err);
...
function assignWork(electricianData, carpenterData, con, res) {
  var sql =
    "select complaint_id,time_slot,catagory,status from complaint_info where status<2";
  con.query(sql, function (err, result) {
    if (err) {
      console.log(err);
      res.end("false");
      ...
      var c = 0;
      for (i in result) {
        if (result[i].time_slot == "10:00-12:00") slot = "slot1";
        if (result[i].time_slot == "13:00-15:00") slot = "slot2";
        if (result[i].time_slot == "16:00-18:00") slot = "slot3";
        if (result[i].catagory == "Staff") {
          e = assignWorkHelper(
            electricianData,
            slot,
            result[i],
        ...
        } else if (result[i].catagory == "Councelor") {
          c = assignWorkHelper(
            carpenterData,
            slot,
            result[i],
      ...
      for (i in electricianData) {
        sql =
          "update handyman_info set slot1=" +
          electricianData[i]["slot1"] +
          ",slot2=" +
          electricianData[i]["slot2"] +
          ",slot3=" +
          electricianData[i]["slot3"] +
          ",issued=" +
          electricianData[i]["issued"] +
          " where handyman_id=" +
          electricianData[i]["handyman_id"];
      ...
      for (i in carpenterData) {
        ...
        sql =
          "update handyman_info set slot1=" +
          carpenterData[i]["slot1"] +
          ",slot2=" +
          carpenterData[i]["slot2"] +
          ",slot3=" +
          carpenterData[i]["slot3"] +
          ",issued=" +
          carpenterData[i]["issued"] +
          " where handyman_id=" +
          carpenterData[i]["handyman_id"];
...
function changeAdminPassword(req, res, con) {
  ...
  con.query(sql, [new_pass, admin_id, old_pass], function (err, result) {
    if (err != null) {
      console.log(err);
      res.end("false");
    } else if (result.affectedRows == 1) res.end("true");
    else res.end("old");
  });
}
...
function adminComplaintAnalysis(req, res, con) {
  var sql =
    "select count(*) as total from complaint_info union select count(*) as solved from complaint_info where status>=2";
...
module.exports = {
  assign: function (req, res, con) {
    ...
    con.query(sql, function (err, result) {
      ...
      sql =
        "select * from handyman_info where Catagory='Staff' and handyman_id in " +
        ids;
      con.query(sql, function (err, result) {
        if (err != null) {
          console.log(err);
          ...
          sql =
            "select * from handyman_info where Catagory='Councelor' and handyman_id in " +
            ids;
          con.query(sql, function (err, result) {
            if (err) {
              console.log(err);
              ...
              for (i in result) carpenterData.push(result[i]);
  ...
  getHandyManRating: function (req, res, con) {
    ...
    //console.log(req.query);
    var sql =
      "select round(avg(rating),2) as rating from complaint_info where handyman_id=" +
      handyman_id +
